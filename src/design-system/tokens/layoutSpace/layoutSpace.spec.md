...
**Dialog body `pb-bottom 48` 是規則 4 line 1 的實作**(預期 body 後接 SurfaceFooter 內 action buttons → universal 套 48),**不是元件特例**。其他容器(Popover / Sheet / generic page)同邏輯:後接 action button → 套 48,不接 → 走 line 2-4 by role。

「**底部 chrome band**」定義:Alert / BulkActionBar / Status bar / Footer 等附著於容器底部的 chrome elements。chrome band **內**含 action buttons → 套 48;chrome band **不**含 action(eg Status bar / 純 Alert)→ 套 loose(line 2)。
