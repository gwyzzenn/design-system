...
| 最後內容 → 容器底 | **規則 4** | (a) 後接 action button?(b) 該容器是 Dialog-class?(c) 元件 role | action button → `bottom`(48)/ Dialog-class 元件 spec own → spec 規則 / bounded region → `loose` / unbounded region → **0** / element → `tight`(generic fallback)|
...
- **判**:
  - 後接 action button?**否**
  - Dialog-class?menu 不是大容器,沒自帶 pb-bottom → **否**
  - role?menu list = **unbounded region**(主導 + 自帶 py 撐)
- **答**:**0**(list 自帶 py 撐 spacing,容器 outer = 0)

### 範例 5:Dialog body 最後 field → body 底

- **走規則**:規則 4
- **判**:
  - 後接 action button(在 SurfaceFooter)?是 → 套 `bottom 48`
  - 同時 Dialog body 自帶 `pb-bottom`(`dialog.spec.md:70`)→ **Dialog-class 元件 spec own**
- **答**:48(由 Dialog body 自帶 `pb-[var(--layout-space-bottom)]` 實現,layout generic rule 跳)

### 範例 6:SurfaceFooter 內 [Cancel] ↔ [Save] gap

- **走規則**:規則 5(橫排並列)
- **判**:Cancel + Save 是 parallel commitment options,不同決策 → **非緊密**
- **答**:`gap-4`(16)— 也可走 button-group.spec own canonical(若 spec 有定 inter-button spacing)
