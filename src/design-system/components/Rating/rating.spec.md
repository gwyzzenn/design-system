...
|------|---------------|-----------|---------|-----------|
| `xs` | **24px**(`h-field-xs`) | 16px | **Standalone 建議值**:商品卡、評論列表旁、搜尋結果 row 的星等展示(非 Field 內時) | — |
| `sm` | 28px(`h-field-sm`) | 16px | Field sm 並排 | field sm |
| `md` | 32px(`h-field-md`) | 16px | **Field 預設**。一般表單評分欄位 | field md |
| `lg` | 36px(`h-field-lg`) | 20px | 送出評分的 review form、強調的主 CTA 區塊 | field lg |

### Standalone vs Field 尺寸選擇(canonical)

- **Standalone 展示**(非 Field 內):用 **`xs`**(24px)—— 對齊 Avatar 24px / Tag sm 20-24px / Material Rating standalone 24dp / iOS HIG 慣例
- **Field 內**(`<Field>` 表單內當 control):跟 Field size 對應傳 sm/md/lg(Field 預設 md)
- 世界級對照:Material Rating standalone = 24dp;Ant Rate 在 Form 內對齊 Form itemSize,standalone 24px。兩種 context 分開設計,不強求單一預設

### Container 對齊 field-height family,icon 走 icon tier

Rating 的 **container 高度消費 `--field-height-*` token**(sm=28 / md=32 / lg=36),讓它可以與 Input / Select / NumberInput / Button 等 field-height family 元件並排時 row-align 一致。**Star icon 大小則走 icon tier**(sm/md=16px, lg=20px,見 `tokens/uiSize/uiSize.spec.md`「Icon 尺寸 Tier」),不跟 container 高度成比例。

### 為什麼不完全對齊 `--field-height-*`

- **現況**:container 高 sm=28 / md=32 / lg=36(完全對齊 `--field-height-*`);star icon 大小 sm=16 / md=16 / lg=20(偏離 control-box tier,走 icon tier)
- **Rationale**:Rating 是**展示型 visual indicator**(語意上是 icon 而非 control box)。container 對齊 field-height 是為了 row-align,但 star 本身走 icon tier——放大到 control-box 尺寸(例如 md=20px、lg=24px)會讓星星在一般卡片視覺過於龐大,失去「評分標記」的語感。Ant Rate / Material MUI Rating 皆走同一流派:star 尺寸獨立於 control-box scale
- **世界級對照**:Ant Design `<Rate>` star 固定 ~20px 不跟 field 成比例;Material MUI `<Rating>` 也有 `size="small/medium/large"` 但 icon 尺寸獨立於 MUI field system
...
| Mode | 觸發 | 行為 | ARIA |
|------|------|------|------|
| **interactive**（預設）| `readOnly={false} && disabled={false}` | hover 預覽、click 設值、鍵盤 Arrow Left/Right/Up/Down 改值、Focus ring | `role="slider"` + `aria-valuenow/valuemin/valuemax` + `tabIndex={0}` |
...
**判斷法**：**送出前 = interactive**，**送出後 / 顯示他人評分 = readOnly**。一顆星 Rating 同時給自己評和看別人評的常見錯誤是都用 interactive——使用者會誤以為可以改別人的分數。
...
Star icon 渲染時明確設 `stroke="none"`(Lucide Star 預設有 1.5px outline stroke,會讓 filled 星星看起來有「深色外框」)。**Rating 是純 fill-only shape**,不保留 outline——對齊 Ant Rate / Material MUI Rating 的世界級慣例。Empty star 靠 `--color-neutral-4` 的 fill 本身與 canvas 對比區隔,不需 outline 補視覺。
