...
## Overlay:Status dot + Count badge(兩種不同 indicator,可並存)

Avatar 支援兩個 overlay,**走不同角、管不同事、可同時存在**。

**關鍵概念釐清**:這**不是疊兩個 Badge**—— status dot 是 Avatar 內部自 render 的 presence indicator(SVG circle + border,**非 `<Badge>` 元件**);count badge 才是消費 `<Badge>`。因此跟「一個 anchor 最多一個 Badge」canonical(`badge.spec.md`)不衝突:Avatar 上**實際只有 1 個 Badge**(count),另一個是非-Badge 的 presence SVG。
