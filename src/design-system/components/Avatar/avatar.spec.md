...
Icon 和 Text 模式使用語義色的 subtle 背景 + 對應前景色。`solid` boolean prop 可切換為 step-6 全色背景 + 白色前景。

與 Tag 元件完全對齊——neutral 用 `foreground`，有色用 **step-7**（非 step-6）優先辨識度：
...
| blue | `--info-subtle` | `--color-blue-7` |
| red | `--error-subtle` | `--color-deep-orange-7` |
| green | `--success-subtle` | `--color-green-7` |
| yellow | `--warning-subtle` | `--color-yellow-7` |
