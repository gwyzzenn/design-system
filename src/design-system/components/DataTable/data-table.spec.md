...
## Column alignment canonical(data type 決定對齊 · 2026-04-22)

**規則**:column 對齊方向由**資料型別**決定,不是 column 位置:

| 資料型別 | align | rationale |
|---------|-------|-----------|
| **Text / Label / Name** | `left` | 閱讀視線起點在左 |
| **Number / Amount / Currency** | **`right`** + `tabular-nums` | decimal alignment 讓眼睛 scan 數量級;tabular 等寬數字對齊 |
| **Date / Time** | `left` | text-like,從左讀起 |
| **Status badge / Pill**(短)| `center`(或 `left` 若 pill 左邊有 icon) | 短 pill center 看起來平衡;consumer 可依 business 選 left |
| **Icon-only col / Checkbox col**(selection / action trigger)| `center` | 無起點文字,center 對齊 row |
| **Action cell**(row actions) | `right` | 慣例:actions 在右側 |

**Header 對齊必跟 cell 對齊一致**(無例外,全業界共識)。

**世界級 benchmark**:

| DS | Text | Number | Date | Status badge | Header vs Cell |
|----|------|--------|------|-------------|---------------|
| Material DataTable | left | **right** | left | left | 一致 |
| Polaris IndexTable | left | **right**(`numeric` col type)| left | left | 一致 |
| Ant Design Table | left | **right** | left | center(常見)| `align` prop 統一 header+cell |
| Atlassian DynamicTable | left | **right** | left | left | 一致 |
| Carbon DataTable | left | **right** | left | center(短 status)| 一致 |
| GitHub Primer | left | right | left | inconclusive | 一致 |
| Apple HIG(macOS Table)| left | **right**(decimal-aligned)| left | inconclusive | 一致 |

**共識 100%**:Text-left / Number-right / Date-left;Status center 只用於**短 pill**,長 status text 依 left 慣例。

**本 DS 實作**:
- `data-table.tsx` 已支援 `align: 'left' | 'center' | 'right'` prop
- Number cell 消費 `tabular-nums`(`font-variant-numeric: tabular-nums`)— code 已落地
- Header `<th>` align 跟 cell 綁定(consumer 傳 column.align,header + cell 都用同一值)

**使用**:

```tsx
const columns = [
  { id: 'name', header: 'Name', align: 'left', cell: (row) => row.name },
  { id: 'amount', header: 'Amount', align: 'right', cell: (row) =>
      <span className="tabular-nums">${row.amount.toFixed(2)}</span> },
  { id: 'date', header: 'Due', align: 'left', cell: (row) => formatDate(row.date) },
  { id: 'status', header: 'Status', align: 'center', cell: (row) =>
      <Tag variant={row.status}>{row.status}</Tag> },
  { id: 'actions', header: '', align: 'right', cell: (row) =>
      <Button iconOnly startIcon={MoreVertical} /> },
]
```

**違反 trigger**:
- ❌ Number col 用 `left` align(失去 decimal alignment,scan 不到數量級)
- ❌ Header 跟 cell align 不一致(視覺 mismatch,column 邊界錯位)
- ❌ Long status text 硬 center(視覺漂浮)
- ❌ Number col 無 `tabular-nums`(不同寬數字鋸齒狀)

**meta pattern**:**語義決定對齊,不是位置** — number 永遠 right 不因為它在第幾欄。對齊邏輯跟 column order 分離。
