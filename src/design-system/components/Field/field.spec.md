...
## Naked variant — Cell-as-input(M19 SSOT)

`naked` = cell-as-input substrate(Notion / Airtable / Excel / AG Grid 共識:host cell 提供 box,Field 融入)。**所有內部 wrapper 必 import + apply `nakedCellRowModeAlign` SSOT**(`field-wrapper.tsx`),自動 propagate cell `data-row-mode`(autoRow→items-start / fixed→items-center)。**反模式 ❌**:wrapper 寫死 `items-center` / 用 `border` 做 state ring(吃 2px 高、corner gap;必 box-shadow inset)。Hook `check_naked_row_mode_propagation.sh` write-time block / `/design-system-audit` M27 batch verify。
