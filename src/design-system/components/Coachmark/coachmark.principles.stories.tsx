...
export const VsPopoverRule: Story = {
  ...
  render: () => (
    <div>
      ...
      <Rule
        title="視覺共用 overlay-surface,差別只有觸發模型 + 結構"
        note="Coachmark 的 bg / border / shadow / radius 完全繼承 Popover(改 Popover 連動)。差別僅:(1) 觸發模型(主動 vs 被動),(2) 結構(Coachmark 無 Header / 有 Media / Footer 是 justify-between)"
      >
  ...
  render: () => (
    <div>
      ...
      <Rule
        ...
        note="破壞性動作(刪除)、必填表單、重要決策用 Dialog。Overlay 強制聚焦,使用者不能忽略,必須完成或取消"
      >
        <Dialog>
      ...
      <Rule
        title="為什麼 onboarding 絕不用 Dialog"
        note="強制彈窗打斷使用者自主探索的動線,心理上造成「又是 tutorial?跳過」的本能反彈。Coachmark 的 non-modal 特性讓使用者保持主控權,tour 反而更容易被完整看完"
      >
        <Label warn>❌ 把 onboarding 包在 Dialog 裡 — 使用者第一反應是找 X 關掉,根本沒讀內容</Label>
        <Label>✅ 用 Coachmark 分步介紹,使用者隨時可 Skip 但多數會跟著走</Label>
      </Rule>

      <Rule
        title="判準 — 問「不看這個資訊會怎樣?」"
        note="不看也能用 → Coachmark(介紹 / 提示 / onboarding)。不看就做不下去(破壞性動作、必要資訊輸入、多步表單)→ Dialog"
      >
