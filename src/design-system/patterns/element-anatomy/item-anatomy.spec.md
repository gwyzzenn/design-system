...
**SidebarMenuButton 獨立實作風險**:SidebarMenuButton 不繼承 MenuItem,直接 import item-anatomy helpers 用 cva 實作。兩者共享同一套公式但實作獨立。改動 MenuItem `menuItemVariants` 後,必 grep `sidebarMenuButtonVariants` 確認對應值是否需要同步。
