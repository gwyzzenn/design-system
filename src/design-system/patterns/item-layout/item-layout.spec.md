...
本 spec 是 **CLAUDE.md 4-Family Model 的 Family 1 + Family 2 SSOT**：
- **Family 1: Menu item layout**（scanning mode，menu 容器內緊湊單列）
- **Family 2: List item layout**（reading mode，頁面上閱讀式單列）

兩者共用結構骨架（`[prefix] [content] [suffix]` + `items-start` + `h-[1lh]` wrapper + 24px 閾值 / card header 規則），差在 density / typography / prefix 大小（見「兩種閱讀模式」章節）。

---

## Family 1: Menu item layout（scanning mode，SSOT）
...
| `FileItem`（detail mode） | `components/FileItem/file-item.tsx` | icon/avatar 作 item boundary,不只是 prefix |
