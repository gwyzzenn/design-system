...
#   (c) `.claude/plugins/design-system/` OR plugin marketplace symlink 不存在
...
# (c) Plugin 已安裝?Anthropic plugin install 路徑 ~/.claude/plugins/<name>/ 或
# project-local .claude/plugins/<name>/(per `docs/en/plugins` install pattern)
PLUGIN_INSTALLED=0
...
偵測:
  ...
  ~/.claude/plugins/design-system/ 或 .claude/plugins/design-system/ 不存在 ❌
