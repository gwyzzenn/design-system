...
現有 `/design-system-audit`(全 dim per design-system-audit SSOT)+ `/product-ui-audit`(7 dim)**全部在 code / spec 層**。code 對、spec 對,**視覺仍可能錯**——這類 bug 在專案歷史反覆出現:
...
/product-ui-audit       consumer UI 對 DS 消費的 7 dim audit(code 層)
