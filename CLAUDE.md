...
**範例最高準則**:精簡幹練、0 重複、每 story earn its existence。Philosophy:舉一反三 / manual 補抽象原則具象化(Jira/Stripe/Notion 劇情)/ 禁湊數秀肌肉(audit Dim 24/25/28/29 抓)。**Earn-existence 2 test**(兩題皆 NO → retire):(a) 教別 story 沒教的原則?(b) 移除後 spec 理解 degrade?**拆分原則**(對齊 Polaris / Carbon / Storybook 官方):不同 affordance 必分(IconOnly / FullWidth)/ AllVariants & AllSizes 對照各 1 / 同 affordance 內 prop variations 用 Controls 不另開(❌ `WithStartIcon`+`WithEndIcon` 拆兩 → ✓ `WithIcon` grid)/ Compound 有 new constraint 才分(`overlayBadge` 必 iconOnly)/ 真實情境 1 故事過 earn-test。**展示 stories 對 trait 取 core**(2026-04-26 v2 trait-based 取代 v1 7-category):元件 spec.md frontmatter 宣告 `traits:` array(hasVariants / hasSizes / hasInteractiveStates / isOverlay / isInputLike / isSelectionMulti / isSelectionSingle / isMatrixHeavy / isStructural / isInternal),required core stories 由 trait 衍生;hook `check_story_category.sh` write-time 攔不符 + audit Dim 29 periodic verify。SSOT → `/story-writing` skill `references/category-templates.md`。
