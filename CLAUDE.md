...
| **M18** | **Propose-time 4 題自檢 gate**(2026-04-25 加)。Claude 列任何 option / 建議 / 候選方案 給 user **前**必 inline 跑 4 題(不靠 user 質疑才回頭驗):**(Q1) M8 benchmark** — ≥ 3 家 world-class 對照(列實作名 / API);**(Q2) M17 SSOT 可傳播** — 是 token / primitive / utility 真 SSOT 還是純 markdown?**(Q3) Rule-of-3 placement** — 概念 ≥ 3 處?選 SSOT,其他 pointer。新加住哪 home?**(Q4) M10 下游吸收** — 加完哪些既有 rule / memory / bug 變冗餘可刪?**reject 的 option 不列出**(或標 fail 原因);通過的 option 必 inline 寫 4-Q 證據表給 user 看。**違反 trigger**:user 問「為什麼會建議這個」/「不是已經有原則嗎」/ user sign-off 後執行才發現選項是錯。**為什麼**:既有 G6/G7/G8 hooks 都是 post-edit / pre-write,**propose-time 沒 gate**;Claude Code event model 沒 OnAssistantMessage hook,只能靠 model-runtime 紀律 + skill mechanical(`/propose-options`)。**M18 vs M12 vs M13 scope**:M12 = canonical rule formation 階段(rule benchmark + invariant test),M13 = user 第 2 次提才 verify trigger,**M18 = 任何 propose 階段**(更上游、更廣)。三者共存,M18 觸發 frequency > M12 > M13。**世界級對照**:Anthropic Claude Code「verify-before-action」/ OpenAI Codex「ground proposals in evidence」/ Cursor docs「propose with confidence + cite source」— ≥ 3 家 AI eng 共識。**實作**:`.claude/skills/propose-options/SKILL.md` 是 SSOT;每次 propose **必 invoke** 自動套 4-Q 表格。 | c hook + d M18-inner-area propose-time 沒 4-Q,user sign-off 前自己回頭驗才撤回(本 conv 2026-04-25) |
