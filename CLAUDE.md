...
| **M17** | **視覺層 vs 幾何層分離**(世界級 DS 的深層 pattern)。User 感知的**視覺盒**與**幾何盒**必須拆成兩層 token / rule,各自走 canonical 不互相污染。5 條落地案例:(1)**Touch target**:視覺盒(icon 16)與 hit 盒(≥24/44)分離 — padding / `::before` 撐開;(2)**Focus ring**:視覺邊界與 ring 幾何分離 — outset + offset,ring 不屬 content box;(3)**Readable width**:container width 與 reading width 分離 — content `max-w-prose` 65ch,容器可更寬;(4)**Icon-text alignment**:icon glyph box 與 optical center 分離 — icon 尺寸基於 cap-height 而非 font-size;(5)**Table alignment**:語義(資料型別)決定對齊,不是位置 — number 永遠 right 不因為它在第幾欄。**根因**:直覺會把視覺盒當成一切,世界級 DS 把它剝離。這是 mindset #2「不憑直覺發明」的深層形式。**違反 trigger**:自訂元件讓 hit 盒 = 視覺盒(a11y 不達標)/ focus ring 放 content box 內(被 clip)/ prose 寬度跟容器綁(readability 破壞)/ icon 用 font-size 算(optical 失衡)/ table number 用 left(失 decimal alignment)。**實作**:各題都有對應 SSOT + benchmark + hook / audit dim(見 `# SSOT 消費 canonical` 清單 5 個新 entries)。 | 2026-04-22 第 2 次 session 補 alignment 類 canonical 發現,5 條獨立規則(target size / focus ring / readable width / optical icon / table alignment)共通 root invariant = 視覺盒 vs 幾何盒分離。若 M17 先存在,新元件建立時會自然拆兩層,不等 audit 才發現 |
...
| **Interactive target size 最小值**(button / icon-target / row-clickable hit area)| `tokens/uiSize/uiSize.spec.md`「Interactive target size floor canonical」— **hit area 最小 24×24**(WCAG 2.5.5 AA);視覺可小(icon 16),hit 盒用 padding / `::before` 撐開。本 DS `--field-height-xs = 24` 是最小 tier |
| **Focus ring 放置**(focus-visible outline placement)| `tokens/color/color.spec.md`「Focus ring 放置 canonical」— 預設 **outset + ring-offset 2px**(`focus-visible:ring-2 focus-visible:ring-offset-2`);dense list / table 可 `ring-inset`;容器 `overflow: hidden` 時 focused item z-index 或 isolation 避免 clip / 壓相鄰 |
| **Readable content max-width**(long-form prose 最大行寬)| `tokens/layoutSpace/layoutSpace.spec.md`「Readable content max-width canonical」— prose / description 多行段落用 **`max-w-prose`**(65ch);UI copy(label / button / tag)不設 max-width |
| **Inline icon + text 對齊**(icon 跟文字怎麼對齊)| `tokens/typography/typography.spec.md`「Inline icon + text 對齊 canonical」— **flex `items-center`** + icon 尺寸略大於 cap-height(`ICON_SIZE` table);不用 baseline-align(icon 下陷) |
| **Table column alignment**(資料型別決定對齊)| `components/DataTable/data-table.spec.md`「Column alignment canonical」— Text-left / **Number-right + tabular-nums** / Date-left / Status-center(短 pill)/ Actions-right。Header 對齊跟 cell 必一致 |
