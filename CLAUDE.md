...
| **hand-craft 繞 DS canonical** | Story / consumer 自刻 `<div relative><input/><div absolute translate-y-1/2><CircularProgress/></div></div>` 繞過 Input `loading` prop / 自刻 `absolute inset-0 flex center CircularProgress` 繞過 `<Empty icon={<CircularProgress/>}/>` / 自刻 `<table>` 繞過 DataTable。視覺對齊在不同 size / density 跑掉;20 dim audit(code/spec 層)抓不到,pixel-level 視覺 audit 需基建。**2026-04-20 Spinner 已廢除遷至 CircularProgress**(世界級命名對齊 Material/Chakra + 支援 determinate) | `# 既有 layout primitives 清單`「自我檢查腳本」;Dim 21 grep signals;memory tech debt #6 視覺 regression 基建 |
