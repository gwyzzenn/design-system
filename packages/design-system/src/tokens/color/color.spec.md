...
### `--on-emphasis` / `--on-emphasis-inverse` 配對:飽和 / 填色底的對比文字
...
- `--on-emphasis-inverse = var(--black-a85)`(深字,dark mode 不反轉)—— 用於**太亮的色底**(白字對比不足):categorical **yellow / amber / orange / lime**。

**合規門檻(2026-06-04 user 拍板「以最低為原則」)= WCAG large/bold 3:1**(solid chip / avatar 的字視為大粗字)。底色 step-6 白字達 3:1 → 用 `--on-emphasis`;不達 → 用 `--on-emphasis-inverse`。

**★ green documented exception**:green-6 白字實測對比 2.47(連 3:1 都不過),但 user 拍板維持白字(沿用世界級慣見的「綠底白字」按鈕觀感)→ 列為知情違反,待日後評估是否調整 green-6 本身。

**`--warning-foreground` 的關係(2026-06-04 改名)**:`--warning-foreground` 現為 `--on-emphasis-inverse` 的 alias,保留供 **warning 語意**「黃底深字」具名取用(API surface)。原本被 categorical yellow/amber 借用 = 誤名(它跟 warning 無關,本質是「亮底深字」通用概念)→ categorical 改消費 `--on-emphasis-inverse`。
...
**例外：step-6 滿版底色上的「亮色 hue」文字用 `--on-emphasis-inverse`（`black-a85`,深色)**——yellow / amber / orange / lime 亮度極高,白字（`--on-emphasis`）連 WCAG 3:1 都不過,必須深色文字（見上「`--on-emphasis` 配對」段)。此例外只在底色是 step-6 時觸發,step-1 subtle 底色上仍用 step-7。（green-6 白字亦不過 3:1,但列 documented exception 維持白字。)
