...
- **Empty(no search results)**:dropdown body 內,非 creatable 時渲 `emptyText`(預設「無符合的選項」);creatable 時保留「建立 "xxx"」row 引導用戶補建選項(由 SelectMenu primitive own,Combobox forward `onCreate`)。
